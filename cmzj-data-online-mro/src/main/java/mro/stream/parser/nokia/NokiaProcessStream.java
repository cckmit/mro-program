...
/**
 * @ClassName NokiaProcessStream
 * @Description //TODO
 * @Author 刘晓雨
 * @Date 2021/4/20 23:08
 * @Version 1.0
 **/
public class NokiaProcessStream extends BroadcastProcessFunction<Map<String, String>, Map<String, String>, String> {
    /**
     * 数据匹配,匹配之后解析入kafka
     */
    SAXReader saxReader;
    ...
    public void processElement(Map<String, String> value, ReadOnlyContext ctx, Collector<String> out) throws Exception {
        if (!"filePath".equals(value.get("filePath"))) {  //？fliepath=文件绝对路径？
            String enodeb_id = value.get("filePath").split("_")[6];//从流中获取基站编号
            /**数据整理*/
...
//                                                System.out.println(listFile.toString().replace("[", "").replace("]", "").trim() + "=" + partitionTo);
                                    out.collect(listFile.toString().replace("[", "").replace("]", "").trim() + "=" + partitionTo); //+"="+partitionTo
                                    partitionTo++;
                                    listFile.clear();
                                    sum = 1;
            ...
            eleLists.stream().filter(e -> !"smr".equals(e.getName())).forEach(el -> {
                ...
                eLists.forEach(ele -> {
                    if (model.equals("A")) {
                        ...
                        int[] indexs = {0, 2, 8, 19, 9, 10, 11, 4, 5, 1, 3, 6, 7, 24, 25, 26, 20, 21, 22, 23, 16, 17, 18, 12, 13, 14, 15, 27, 28};

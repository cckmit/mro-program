...
import mro.stream.source.eric.CollectEricFileFtp;
import mro.stream.source.eric.InitialEricData;
 ...
 **/
public class MrEricTransformations {
    ...
    public static void main(String[] args) throws Exception {
        ...
        BroadcastStream<Map<String, String>> broadcastStream = env.addSource(new EricHdfsSource()).setParallelism(1).broadcast(CONFIG_KEYWORDS);
        /**自定义数据源，实现循环一直下载SFTP文件，内部实现资源竞争*/
        DataStream<Map<String, String>> listdata = env.addSource(new CollectEricFileFtp()).setParallelism(4);

        /**filter过滤测试流*/
        SingleOutputStreamOperator<String> map = listdata.connect(broadcastStream)
                .process(new EricProcessStream())

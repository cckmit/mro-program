...
public class CollectDtTools {
    ...
    public static String getServeInfo(JedisCluster jedis) {
        ...
        while (true) {
            try {
                ...
                //进行能否连接ftp验证
                if (new FtpUtil(false).login(sftpHost, sftpPort, sftpUser, sftpPass)) {     //涉及事务，有可能同时插入，插入失败的原因有很多，有可能事务没提交，需要在提交插入
    ...
    /**
     * 生成文件时间(2021-04-20)，和拼接文件的时间戳(20210420233000)
     */
     ...
     * @Author 刘晓雨
     * @Description //到数据库查询对应的ts_format字段
     * @Date 0:03 2021/4/21
    ...
    public static Map<String, String> extract(Connection conn, String ip, Map<String, String> map, String data) {
        try {
            ...
            System.out.println("MR数据库查询ts_format字段报错，报错信息：" + e.toString());
        }
        return map;
    }
